...
>
	<article class="relative lg:w-9/12">
		<!-- ! NAVBAR -->
		<menu
			class="
			{scrollY > bottomContentContainerTop + bottomContentContainerOffsetHeight ? 'fixed' : 'hidden'}
			w-postNav top-0 z-50 flex  h-20 items-end border-b border-neutral-300 bg-neutral-50 
			"
		>
		...
		<div class="flex flex-col-reverse md:flex-col">
			...
			<div class="mx-4 flex items-center space-x-2 py-2 md:mx-0 md:hidden">
				...
				>
					<Heart classNames="h-4 w-4 {liked ? 'stroke-red-500' : 'stroke-neutral-800'}" outline />
				...
				<button
					...
					on:click={() => updateLikes(data?.data.id)}
				>
					<Share classNames="h-4 w-4 {liked ? 'stroke-red-500' : 'stroke-neutral-800'}" outline />
				...
				<div class="flex w-full flex-col">
					...
					<div class="mt-3 flex items-center space-x-3 opacity-75 md:mt-2 md:space-x-4">
						<div class="text-sm">
							...
							<p class="">Kategoria:<a class="ml-1 underline" href="#">{data.data.category}</a></p>
						</div>
		...
		>
			...
			<div class="hidden flex-col items-center md:sticky md:top-24 md:left-4 md:flex md:w-2/6">

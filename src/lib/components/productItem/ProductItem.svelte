	...
	>
		...
		>
			<a href={postURL} class="h-full w-full">
				...
				/> -->
				{#if product.images && product?.images[0]?.publicId !== undefined}
					<img
						use:image={{
						...
						class="
						transition-bezier h-full w-full object-cover group-active:scale-110 lg:group-hover:scale-110 lg:group-active:scale-100"
						alt={product.title}
					/>
				{:else}
					<div
						class="flex h-full w-full items-center justify-center bg-gradient-to-t from-neutral-300 to-neutral-100 dark:from-neutral-700 dark:to-neutral-800"
					>
	...
	<section class="relative my-4 mb-6 flex h-auto w-full items-center px-2 ">
		<div class=" minWidthNonCard left-0 h-full cursor-pointer overflow-hidden rounded-md">
			<a href={postURL}>
				...
				{:else}
					<div
						class="flex  h-44 min-h-full w-full items-center justify-center bg-gradient-to-t from-neutral-200 to-neutral-100  dark:from-neutral-700 dark:to-neutral-800"
					>
						<ImageIcon class="mx-auto h-12 w-12 text-neutral-400 dark:text-neutral-500" />
					</div>
				{/if}

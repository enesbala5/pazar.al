import { c as create_ssr_component, b as add_attribute, v as validate_component, e as escape, d as subscribe } from "../../chunks/index.js";
...
const Nav = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<nav class="${"flex items-center justify-between p-4"}"><section class="${"flex items-center"}"><a href="${"/"}" class="${"h-8 w-8 "}">${validate_component(Logo, "Logo").$$render(
	...
	<div class="${"flex items-center "}">
		<div class="${"mr-4 flex items-center space-x-4 border-r-2 " + escape(onIndex ? " border-neutral-700" : " border-neutral-800", true) + " px-4 pr-4"}"><div class="${"flex h-5 w-5 items-center justify-center opacity-70"}">${validate_component(Heart, "Heart").$$render($$result, { classNames: "fill-white" }, {}, {})}</div>
			<div class="${"flex h-5 w-3.5 items-center justify-center opacity-70"}">${validate_component(Save, "Save").$$render($$result, { classNames: "fill-white" }, {}, {})}</div></div>
		
		

		
		<div class="${"flex h-10 w-10 items-center justify-center rounded-full " + escape(onIndex ? " bg-neutral-900" : " bg-neutral-800", true)}"><p class="${"mb-0.5 text-xl text-neutral-50"}">E</p></div></div></nav>`;
});
const app = "";
